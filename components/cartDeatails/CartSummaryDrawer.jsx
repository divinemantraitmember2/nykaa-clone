...
<div className="h-[calc(100%-140px)] overflow-y-auto px-4 py-2">
        {items.map((item, index) => (
          <div key={index} className="flex gap-4 px-2 py-4 border-b border-gray-200 relative">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-semibold text-gray-800 mt-1">₹{item.price_inr} × {item.quantity} = ₹{item.price_inr * item.quantity}</p>
              <div className="flex items-center gap-2 mt-3">
                <button onClick={() => updateCartQuantity(item.id, item.quantity - 1)} className="w-8 h-8 text-lg bg-gray-100 hover:bg-gray-200 rounded-md font-bold">−</button>
                <span className="font-semibold text-gray-800">{item.quantity}</span>
                <button onClick={() => updateCartQuantity(item.id, item.quantity + 1)} className="w-8 h-8 text-lg bg-gray-100 hover:bg-gray-200 rounded-md font-bold">+</button>
              </div>
            </div>
            <button onClick={() => removeItem(item.id)} className="absolute top-3 right-3 text-red-600 hover:text-red-800">
              <FaTrash />
            </button>
          </div>
        ))}
        ...
        <div className="mt-6 space-y-3 bg-gray-50 p-4 rounded-lg shadow-inner ">
          ...
          <div className="flex justify-between text-sm text-gray-700">
            ...
            <span className="text-green-600">₹{CartResponse?.totalDiscount.toFixed(2)}</span>
          </div>
          <div className="border-t border-gray-200 my-2"></div>
          <div className="flex justify-between font-semibold text-lg text-gray-900">
            <span>You Pay</span>
            <span>₹{CartResponse?.totalPrice.toFixed(2)}</span>
          </div>
        </div>
