    ...
    <div className="w-full bg-white py-6 px-3 sm:px-4 md:px-6">
      ...
      <div className="flex gap-4 overflow-x-auto sm:grid sm:grid-cols-2 md:grid-cols-3 sm:gap-6 max-w-6xl mx-auto scroll-smooth snap-x snap-mandatory sm:overflow-visible">
        {items.map((item, idx) => (
          <div
            key={idx}
            className="min-w-[250px] sm:min-w-0 shrink-0 snap-start group bg-white p-4 flex flex-col items-center text-center border border-gray-200 rounded-lg transition hover:shadow-md hover:-translate-y-1 duration-300"
          >
            <div className="mb-4">
              <Image
                src={item.src}
